...
const messages = Messages.loadMessages('@salesforce/plugin-generate', 'messages');
...
export class MessageUtil {
  ...
  public static buildDescription(
    ...
    return messages.getMessage(descriptionKey, tokens) + '\n' + this.getHelpHead(isLightningBundle) + (extra || '');
  }
  ...
  public static buildHelpText(examples: string[], isLightningBundle: boolean, extra?: string): string {
    return (
      ...
      (extra || '') +
      messages.getMessage('HelpExamplesTitle') +
  ...
  private static getHelpHead(isLightning: boolean): string {
    return (
      messages.getMessage('HelpDefaults') +
      messages.getMessage('HelpOutputDirRelative') +
